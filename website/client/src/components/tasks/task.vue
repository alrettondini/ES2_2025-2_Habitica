...
export default {
  ...
  methods: {
    ...
    async duplicateTask () {
      ...
      // Criar uma cópia da task removendo campos que não devem ser copiados
      const taskCopy = {
        type: this.task.type,
      ...
      if (this.task.checklist && this.task.checklist.length > 0) {
        taskCopy.checklist = this.task.checklist.map(item => ({
          text: item.text,
          completed: false, // Resetar checklist items
        }));
      }

      // Adicionar propriedades específicas por tipo de task
      if (this.task.type === 'habit') {
        taskCopy.up = this.task.up !== false; // default true
        taskCopy.down = this.task.down !== false; // default true
      ...
      try {
        ...
        // Forçar reload das tasks para garantir que aparece na interface
